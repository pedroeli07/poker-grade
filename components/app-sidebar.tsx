...
function NavItem({
  ...
  return (
    <Link
      href={href}
      className={cn(
        "group relative flex items-center gap-2.5 rounded-lg transition-all duration-300 overflow-hidden",
    ...
    >
      ...
      <span
        className={cn(
          "relative z-10 flex shrink-0 items-center justify-center rounded-md transition-all duration-300",
          isOpen ? "h-8 w-8" : "h-9 w-9",
          nested && isOpen && "h-7 w-7",
          active
            ? "bg-primary/20 text-primary"
            : "bg-blue-500/10 text-muted-foreground group-hover:bg-muted/80 group-hover:text-foreground"
        )}
      >
        <Icon className={cn(nested && isOpen ? "h-3.5 w-3.5" : "h-4 w-4")} />
      </span>
      ...
      {isOpen && (
        <span className="relative z-10 flex flex-1 items-center gap-2 min-w-0">
          <span
            className={cn(
              "text-[14px] font-medium tracking-[-0.01em] truncate transition-colors duration-200",
              nested && "text-[13px]",
    ...
    <Collapsible open={expanded} onOpenChange={setExpanded}>
      <CollapsibleTrigger asChild>
        <button
          type="button"
          className={cn(
            "group relative flex w-full cursor-pointer items-center gap-2.5 rounded-lg px-2.5 py-1.5 transition-all duration-300 overflow-hidden text-left",
        ...
        >
          ...
          <span
            className={cn(
              "relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-md transition-all duration-300",
              anyChildActive
                ? "bg-primary/20 text-primary"
                : "bg-blue-500/10 text-muted-foreground group-hover:bg-muted/80 group-hover:text-foreground"
            )}
          ...
          <span className="relative z-10 flex flex-1 items-center gap-2 min-w-0">
            <span
              className={cn(
                "text-[14px] font-medium tracking-[-0.01em] truncate",
      ...
      <CollapsibleContent className="space-y-0.5 pt-0.5 pl-1.5 ml-4 border-l border-primary/15">
        {group.items.map((child) => (
          <NavItem
            key={child.href}
            href={child.href}
...
function AppSidebar({ userRole }: { userRole: UserRole }) {
  ...
  return (
    ...
    >
        ...
        {isOpen ? (
          <>
            ...
            <div className="flex flex-col leading-none flex-1 min-w-0">
              <span className="text-[13px] font-black tracking-[0.16em] uppercase text-primary">
                CL
              </span>
              <span className="text-[16px] font-black tracking-tight text-primary leading-tight">
                TEAM
              </span>
            </div>
            <button
              ...
              className="bg-blue-500/20 hover:bg-blue-500/30 text-muted-foreground/80 hover:text-foreground transition-colors p-1 rounded-md  shrink-0 cursor-pointer"
            >
          ...
          <button
            ...
            className="relative shrink-0 flex h-8 w-8 items-center justify-center rounded-lg bg-primary/10 text-primary transition-colors hover:bg-primary/15 cursor-pointer"
          >
            <Spade className="h-4 w-4" />
          </button>
        )}
      ...
      <nav
        className={cn(
          "sidebar-scrollbar flex-1 overflow-y-auto overflow-x-hidden px-1.5 py-3 space-y-0.5"
        )}
      >
        {isOpen && (
        ...
        <div className="my-1.5 mx-2.5 h-px bg-border/50" />
