...
- Página `/admin/jogadores` — lista de jogadores com botão "Novo Jogador" (formulário com campos: Nome Completo, Nickname único, E-mail, Coach Responsável, Grade principal, ABI alvo)
- Página `/admin/grades/perfis` — grades de torneios importadas da Lobbyze via JSON
- Página `/admin/grades/importacoes` — importação de histórico de torneios via Excel (.xlsx, .xls, .csv) da plataforma Lobbyze
- Página `/admin/grades/revisao` — conferência de torneios extra-play e suspeitos
- Página `/admin/grades/metas` — targets com gatilhos de subida/descida de limite
- Página `/admin/grades/historico` — histórico de movimentações de limite
- Página `/admin/usuarios` — gerenciamento de usuários e convites
...
### /admin/jogadores — adicionar coluna "Performance 10d"
...
### /admin/jogadores/[id] — adicionar seção SharkScope
...
### /admin/jogadores — modal "Novo Jogador"
O modal atual tem um único campo "Nickname" usado para identificação interna. **Manter esse campo exatamente como está** — ele não cria `PlayerNick` automaticamente. Os nicks por rede de poker são adicionados pelo usuário na ficha individual do jogador (`/admin/jogadores/[id]`) após o cadastro, usando o formulário "Adicionar nick".

### /admin/dashboard (Overview) — atualizar card "Aderência de Grade"
