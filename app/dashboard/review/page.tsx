  ...
  return (
    <div className="space-y-6">
        ...
        {pendingReviews.length > 0 && (
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/20">
            ...
            <span className="text-sm font-semibold text-amber-400">
              {pendingReviews.length} pendente
      ...
      {pendingReviews.length === 0 ? (
        ...
        <div className="space-y-6">
          {groups.map(({ player, reviews }) => (
            <div
              key={player.id}
              className="rounded-xl border border-border overflow-hidden bg-primary/5 shadow-sm"
            >
              {/* Player section header */}
              <div className="flex items-center gap-4 px-5 py-4 bg-primary/10 border-b border-border/60">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${avatarColor(player.name)}`}
                >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <Badge
                      variant="outline"
                      className="border-amber-500/30 text-amber-500 bg-amber-500/8 text-sm px-2.5"
                    >
              ...
              <div className="divide-y divide-border/60">
                {reviews.map((review) => (
                  <div
                    key={review.id}
                    className="flex items-center gap-5 px-6 py-4 hover:bg-primary/10 transition-colors group"
                  >
                    {/* Tournament info */}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        {review.tournament.matchStatus === "SUSPECT" ? (
                          <Badge className="bg-amber-500/10 text-amber-500 border-amber-500/25 border text-xs px-2">
                            Suspeito
                          </Badge>
                        ) : (
