...
/** Contas de mesa na UI; exclui `PlayerGroup` (nick sintético do grupo SharkScope). */
export function filterNicksExcludingPlayerGroup<T extends { network: string }>(
  nicks: T[] | undefined
): T[] {
  return nicks?.filter((n) => n.network !== "PlayerGroup") ?? [];
}
