...
export async function uploadTournaments(formData: FormData): Promise<UploadResult> {
  ...
  importsQueriesLog.info("Arquivo recebido", { fileName: file.name, sizeKB: Math.round(file.size / 1024), player: player.name });
  ...
  try {
    ...
    void notifyImportBatchExternal({
      ...
      summaryLines: [`${player.name}: ${allTournaments.length} torneios — ${playedCount} jogados, ${extraPlayCount} extra play, ${didntPlayCount} não jogados`],
    });
    ...
    return {
      ...
      summary: [`${player.name}: ${allTournaments.length} torneios — ${playedCount} jogados, ${extraPlayCount} extra play, ${didntPlayCount} não jogados`],
    };
  } catch (err) {
    if (isRedirectError(err)) throw err;
